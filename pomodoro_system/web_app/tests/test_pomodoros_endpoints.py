...
class TestBeginPomodoroAPI:
    ...
    def test_begin_pomodoro_without_authentication_header(self, client: testing.FlaskClient, orm_task):
...
class TestPausePomodoroAPI:
    ...
    def test_pause_pomodoro_with_valid_data(self, client: testing.FlaskClient, started_orm_pomodoro,
                                            project_owner_authorization_token):
        response = client.post(f'/pomodoros/{started_orm_pomodoro.id}/pause',
                               headers={'Authorization': project_owner_authorization_token}, json={})
    ...
    def test_pause_pomodoro_without_authentication_header(self, client: testing.FlaskClient, started_orm_pomodoro):
...
class TestResumePomodoroAPI:
    ...
    def test_resume_pomodoro_with_valid_data(self, client: testing.FlaskClient, paused_orm_pomodoro,
                                             project_owner_authorization_token):
        response = client.post(f'/pomodoros/{paused_orm_pomodoro.id}/resume',
                               headers={'Authorization': project_owner_authorization_token}, json={})

        expected_end_date_utc_hour = datetime.now(tz=pytz.UTC).hour
        repo = SQLPomodoroRepository()
        fetched_pomodoro = repo.get(paused_orm_pomodoro.id)
    ...
    def test_resume_pomodoro_without_authentication_header(self, client: testing.FlaskClient, paused_orm_pomodoro):
        response = client.post(f'/pomodoros/{paused_orm_pomodoro.id}/resume', headers={}, json={})

        assert response.status_code == 401

    def test_resume_pomodoro_with_non_authorized_user(self, client: testing.FlaskClient, paused_orm_pomodoro,
                                                      random_project_owner_authorization_token):
        response = client.post(f'/pomodoros/{paused_orm_pomodoro.id}/resume',
                               headers={'Authorization': random_project_owner_authorization_token}, json={})
...
class TestFinishPomodoroAPI:
    @db_session
    def test_finish_pomodoro_with_valid_data(self, client: testing.FlaskClient, started_orm_pomodoro,
                                             project_owner_authorization_token):
        response = client.patch(f'/pomodoros/{started_orm_pomodoro.id}/finish',
                                headers={'Authorization': project_owner_authorization_token}, json={})

        expected_end_date_utc_hour = datetime.now(tz=pytz.UTC).hour
        repo = SQLPomodoroRepository()
        fetched_pomodoro = repo.get(started_orm_pomodoro.id)

        assert response.status_code == 200
        assert fetched_pomodoro.is_finished
        assert fetched_pomodoro.end_date.hour == expected_end_date_utc_hour

    def test_finish_pomodoro_without_authentication_header(self, client: testing.FlaskClient, started_orm_pomodoro):
        response = client.patch(f'/pomodoros/{started_orm_pomodoro.id}/finish',
                                headers={}, json={})

        assert response.status_code == 401

    def test_finish_pomodoro_with_non_authorized_user(self, client: testing.FlaskClient, started_orm_pomodoro,
                                                      random_project_owner_authorization_token):
        response = client.patch(f'/pomodoros/{started_orm_pomodoro.id}/finish',
                                headers={'Authorization': random_project_owner_authorization_token}, json={})
